...
[--arch=ARCH] [--driverVersion=VERSION] [--llvmVersion=VERSION]
...
that based on GCN1.0/1.1/1.2 architecture and it can generate AMD Catalyst
OpenCL binaries and GalliumCompute OpenCL binaries. It is compatible with GNU assembler
...
Enable generating of 64-bit binaries (only for AMD catalyst format).
