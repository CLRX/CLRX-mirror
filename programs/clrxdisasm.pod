...
This is CLRadeonExtender utility to disassemble Radeon GPU code.
This disassembler can disassemble code for GCN 1.0/1.1/1.2/1.4 (AMD VEGA)
architectures, but not for VLIW architecture.

Disassembler can handle AMD Catalyst(tm) OpenCL(tm) kernel binaries and the
